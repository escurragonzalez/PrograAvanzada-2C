...
public class ConexionRed {
	...
	public ConexionRed(String path) throws FileNotFoundException {
		...
		int inicio, fin;
		char c = ' ';
		//Como no se la cantidad de nodos hasta leer todo el archivo para prodarlo lo deje en un numero arbitrario
		war = new Warshall(25);
		result = new char[25];
		cantPedidos = 0;
		while (c != 'F' && sc.hasNext()) {
			...
			if (c != 'F') {
				...
				if (c == 'P') {
					war.resolver();
					//Resuleve cuando lee para saber si hay camino es por la consigna en ese momoento si hay o no camino
					//no se tiene la info de la red completa, es sobre el paso a paso 
